...
def forecast_product(product: Dict[str, Any], forecast_days: int = FORECAST_DAYS) -> Dict[str, Any]:
    pid     = to_int(product["id"])
    stock   = to_int(product["current_stock"])
    reorder = to_int(product["reorder_level"])
    days    = max(1, min(int(forecast_days or FORECAST_DAYS), 90))
    ...
    if len(out_df) < MIN_TRAINING_RECORDS:
        ...
        predicted = round(avg_daily * days, 2)
        return {
            "product_id":    pid,
    ...
    future_dates = [datetime.now().date() + timedelta(days=i) for i in range(1, days + 1)]
...
def health_check():
    try:
        ...
        row = cursor.fetchone()
        count = row["product_count"] if isinstance(row, dict) else row[0]
        cursor.close()
        conn.close()
...
def run_forecast(request: ForecastRequest = ForecastRequest()):
    try:
        horizon = max(1, min(int(request.horizon_days or FORECAST_DAYS), 90))
        products  = get_all_products()
        forecasts = []
        errors    = []

        print(f"\n[FORECAST] Starting for {len(products)} products ({horizon}-day horizon)...")

        for p in products:
            try:
                result = forecast_product(p, horizon)
