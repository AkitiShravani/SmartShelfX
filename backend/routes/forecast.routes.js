...
router.post('/run', requireRole('ADMIN', 'MANAGER'), async (req, res) => {
    try {
        ...
        const horizonDays = Number(req.body?.horizon_days) || 7;

        const mlResponse = await axios.post(`${mlUrl}/forecast`, { horizon_days: horizonDays }, { timeout: 60000 });
                    ...
                    if (['HIGH', 'CRITICAL'].includes(item.risk_level)) {
                        ...
                        if (product) {
                            ...
                            await Alert.create({
                                ...
                                message: `AI Forecast: ${product.name} (${product.sku}) — Risk: ${item.risk_level}. Predicted demand: ${Math.ceil(item.predicted_qty)} units in next ${horizonDays} days. Current stock: ${product.current_stock}.`,
                                is_read: false
                            });
