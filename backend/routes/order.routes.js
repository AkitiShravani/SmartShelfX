...
const { PurchaseOrder, Product, User, StockTransaction, sequelize } = require('../models');
...
router.put('/:id/status', async (req, res) => {
    try {
        ...
        if (status === 'DELIVERED' && previousStatus !== 'DELIVERED') {
            ...
            try {
                const product = await Product.findByPk(order.product_id, { lock: t.LOCK.UPDATE, transaction: t });
                if (!product) {
                    await t.rollback();
                    return res.status(404).json({ error: 'Product not found for this order' });
                }

                const newStock = product.current_stock + Number(order.quantity);
