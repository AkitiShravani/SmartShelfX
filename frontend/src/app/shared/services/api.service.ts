...
export class ApiService {
    ...
    runForecast(horizonDays = 7): Observable<{ success: boolean; message: string; forecasts: ForecastResult[] }> {
        return this.http.post<any>(`${this.API}/forecast/run`, { horizon_days: horizonDays });
    }
    ...
    getVendors(): Observable<any[]> {
        return this.http.get<any[]>(`${this.API}/auth/vendors`);
    }
}